...
public class PlayerMovement : MonoBehaviour {
    ...
    private void MyInput() {
        ...
        if (x != 0 || y != 0)
        {
            itemParentAnimator.SetBool(moveHash, true);
        }
        else
        {
            itemParentAnimator.SetBool(moveHash, false);
        }
    ...
    private void StartCrouch() {
        ...
        transform.position = new Vector3(tp.x, tp.y - 0.5f, tp.z);
    ...
    private void CounterMovement(float x, float y, Vector2 mag) {
        ...
        if (Math.Abs(mag.x) > threshold && Math.Abs(x) < 0.05f || (mag.x < -threshold && x > 0) || (mag.x > threshold && x < 0)) {
            rb.AddForce(acceleration * orientation.transform.right * Time.deltaTime * -mag.x * counterMovement);
        }
        if (Math.Abs(mag.y) > threshold && Math.Abs(y) < 0.05f || (mag.y < -threshold && y > 0) || (mag.y > threshold && y < 0)) {
            rb.AddForce(acceleration * orientation.transform.forward * Time.deltaTime * -mag.y * counterMovement);
        }
